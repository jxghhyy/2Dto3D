...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-disparity", type=float, default=32, help="低分辨率下最大水平视差像素")
...
# ============================================================
# 【师兄核心优化 1】int64 无损 Z-buffer DIBR
# 用位操作编码深度+坐标，彻底消除浮点精度问题导致的伪影
# ============================================================
@torch.no_grad()
def forward_warp_right_gpu(
    ...
    t0 = time.perf_counter()
    # ========== 师兄 int64 无损 Z-buffer 算法 ==========
    # 把 near_score（深度）和 src_lin（坐标）编码到同一个 int64
    # 高位：深度（离镜头越近越大），低位：坐标
    # 这样直接比较整数大小，彻底消除浮点精度问题！
    NEAR_BITS = 20
...
# ============================================================
# 【师兄核心优化 2】空洞左侧膨胀
# 保护前景边缘不被 inpaint 侵蚀，解决前景边缘发虚问题
# ============================================================
@torch.no_grad()
...
def main() -> None:
    ...
    for idx, filename in enumerate(files):
        ...
        print(f"[mono2stereo] 🚀 【师兄优化】int64无损DIBR + 前景边缘保护 (hole-dilate-left={args.hole_dilate_left})")
        ...
        # ==================== 视差像素跨模式归一化 ====================
        # 用户输入的 --max-disparity 表示"最终输出画面（原分辨率）上的像素偏移"。
        # video-model 模式：DIBR 工作分辨率 = 原分辨率，直接用 args.max_disparity
        # 单帧模式：DIBR 工作在 low_w 上，要按比例缩到 low_w，否则两个模式 3D 强度差一倍
        dibr_work_w = w_orig if args.video_model else low_w
        effective_max_disparity = args.max_disparity * dibr_work_w / w_orig
        print(f"[mono2stereo] max-disparity user={args.max_disparity:.1f}px (on original w={w_orig}) "
              f"→ DIBR work disp={effective_max_disparity:.2f}px (on w={dibr_work_w}, "
              f"video_model={args.video_model})")
        ...
        try:
            while True:
                ...
                right_inpainted_low = fast_inpaint_gpu(
                    right_rgb_low,
                    hole_mask=hole_dilated,  # 用膨胀后的空洞
                ...
                t0 = time.perf_counter()
                if args.video_model:
                    # Video-DAV2: DIBR 已经是原分辨率，不需要上采样
                    right_u8 = right_u8_low
                else:
                    # DAV2: 低分辨率结果需要上采样回原分辨率
