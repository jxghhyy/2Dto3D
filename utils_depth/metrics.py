...
def compute_siou(pred, target, left):
    ...
    pred_i   = pred.astype(np.int16)
    target_i = target.astype(np.int16)
    left_i   = left.astype(np.int16)

    diff_gl = np.abs(pred_i   - left_i).astype(np.uint8)
    diff_rl = np.abs(target_i - left_i).astype(np.uint8)
    # ------------------------------------------------------------------
...
def cropping_img(args, pred, gt_depth):
    ...
    if args.dataset == 'kitti':
        if args.do_kb_crop:
            ...
            gt_depth = gt_depth[top_margin:top_margin + 352,
