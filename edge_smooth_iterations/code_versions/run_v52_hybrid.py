...
核心思路：
1. 前 N 次迭代：纯从右向左卷积（防渗色，保护前景）
   - 背景永远在空洞右侧，优先用背景填充
2. 剩余空洞：双向不对称核（处理大空洞，快速填充）

这样既保留从右向左的防渗色优点，又能处理 1000px+ 的大空洞
"""
import sys
import cv2
import numpy as np
...
KERNEL_SIZE = 15
RIGHT_FIRST_ITERS = [3, 4, 5, 6]  # 测试不同的前 N 次纯向右填充
...
@torch.no_grad()
def inpaint_hybrid(img, hole, kernel_right, kernel_bidir, right_first_iters=5, max_total=8):
    """
    混合填充策略
    1. 前 right_first_iters 次：纯从右向左填充（防渗色）
    2. 剩余：双向不对称核填充（处理大空洞）
    """
    result = img.clone()
    hole_cur = hole.clone()
    ...
    for it in range(max_total):
        ...
        fill_counts.append((mode, fill_count))
...
def main():
    ...
    print(f"[v52 混合填充] 设备: {device}")
    print(f"[v52 混合填充] 卷积核尺寸: {KERNEL_SIZE}")
    print(f"[v52 混合填充] 测试纯向右填充轮数: {RIGHT_FIRST_ITERS}")
    ...
    OUT_DIR = Path("/mnt/A/jiangxg/work/2Dto3D/edge_smooth_iterations/frames/v52_hybrid_test")
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    kernel_right = create_right_only_kernel(KERNEL_SIZE, device)
    kernel_bidir = create_bidirectional_kernel(KERNEL_SIZE, device, 5)

    results = []
    for n in RIGHT_FIRST_ITERS:
        print(f"\n测试前 {n} 次纯向右填充...")
        t0 = time.time()
        result, hole_final, fill_counts = inpaint_hybrid(
            right_warped, hole_dilated, kernel_right, kernel_bidir, n, 8
        )
        t1 = time.time()
    ...
    print("\n" + "=" * 60)
    print("【混合填充策略对比】")
    print(f"{'N(向右)':<10} {'剩余':<10} {'耗时(ms)':<12} {'每轮填充数'}")
    print("-" * 60)
    ...
    # 颜色对比
    print("\n【背景区域颜色对比】")
    v51 = cv2.imread("frames/v51_fixed_verify/crop_shoulder.png")
    v51_bg = v51[50:150, 150:250]
    print(f"v51 (双向): R={v51_bg[:,:,2].mean():.1f}, G={v51_bg[:,:,1].mean():.1f}, B={v51_bg[:,:,0].mean():.1f}")
    for n in RIGHT_FIRST_ITERS:
