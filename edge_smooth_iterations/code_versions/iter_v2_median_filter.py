...
v1 的问题：高斯滤波会被离群点污染
v2 改进：
  1. 先剔除边界位置的离群点（用中位数 ± 3σ）
  2. 用中值滤波代替高斯滤波（对跳变更鲁棒）
"""
import sys
import argparse
import cv2
...
def parse_args():
    ...
    parser.add_argument("--median-kernel", type=int, default=7, help="垂直中值滤波核大小")
    return parser.parse_args()


# ========== v2 改进：中值滤波 + 离群点剔除 ==========
def project_disocclusion_bands_median_smooth(disparity, min_drop=3.0, right_cleanup=16, median_kernel=7):
    """
    v2 改进版：
    1. 先生成原始的反遮挡带
    2. 提取每行的左边界位置
    3. 先剔除离群点（中位数 ± 3σ）
    4. 用一维中值滤波做垂直平滑（对跳变更鲁棒）
    """
    h, w = disparity.shape
    if w < 2:
    ...
    edge_map = torch.full((h,), w, device=disparity.device, dtype=torch.float32)

    for y in range(h):
        # 找这一行第一个从 0 变 1 的位置
        row = band_float[y]
        edges = torch.where(row[1:] - row[:-1] > 0.5)[0]
        if len(edges) > 0:
            edge_map[y] = edges[0].float() + 1  # +1 因为我们看的是 i+1 - i

    # ========== 离群点剔除 ==========
    # 用中位数 + 绝对偏差 找出异常值
    valid_mask = edge_map < w - 1
    if valid_mask.sum() > 10:
        valid_values = edge_map[valid_mask]
        median = torch.median(valid_values)
        mad = torch.median(torch.abs(valid_values - median))  # 绝对偏差中位数
        threshold = 3.0 * mad  # 3σ 原则

        # 离群点 = 和中位数差 > 3*MAD
        outlier_mask = torch.abs(edge_map - median) > threshold
        edge_map[outlier_mask] = median  # 用中位数填充离群点

    # ========== 垂直中值滤波 ==========
    # 转换为 [N,C,H,W] 格式
    edge_1d = edge_map.view(1, 1, h, 1)

    # 中值滤波（通过 unfold + median 实现）
    pad = median_kernel // 2
    edge_padded = F.pad(edge_1d, (0, 0, pad, pad), mode='replicate')

    # unfold 得到每个位置的邻域
    kernel_size = median_kernel
    unfolded = F.unfold(edge_padded, kernel_size=(kernel_size, 1), padding=0, stride=1)  # [1, K, H]
...
def process_frame(frame_bgr, model, device, args):
    ...
    # C版本：v2 改进的中值滤波平滑
    disocclusion_band = project_disocclusion_bands_median_smooth(
        disparity_sharp, min_drop=3.0, right_cleanup=16, median_kernel=args.median_kernel
    )
...
def main():
    ...
    print(f"设备: {device}, 版本: v2 中值滤波 + 离群点剔除 (kernel={args.median_kernel})")
    ...
    cv2.imwrite(str(outdir / 'v2_final_right.png'), cv2.cvtColor(final_u8, cv2.COLOR_RGB2BGR))

    band_viz = (result['warped'] * 255).astype(np.uint8)
    band_viz[result['disocclusion_band']] = [0, 255, 0]
    cv2.imwrite(str(outdir / 'v2_band_green.png'), cv2.cvtColor(band_viz, cv2.COLOR_RGB2BGR))

    hole_viz = (final_u8.copy() * 0.5).astype(np.uint8)
    hole_viz[result['hole_after']] = [0, 0, 255]
    cv2.imwrite(str(outdir / 'v2_hole_red.png'), cv2.cvtColor(hole_viz, cv2.COLOR_RGB2BGR))

    # 边缘裁剪
    y1, y2, x1, x2 = 300, 700, 900, 1100
    cv2.imwrite(str(outdir / 'v2_final_edge_crop.png'),
                cv2.cvtColor(final_u8[y1:y2, x1:x2], cv2.COLOR_RGB2BGR))
    cv2.imwrite(str(outdir / 'v2_band_edge_crop.png'),
                cv2.cvtColor(band_viz[y1:y2, x1:x2], cv2.COLOR_RGB2BGR))

    # ========== 定量评估 ==========
    print("\n" + "="*60)
    print(f"📊 v2 中值滤波版本 (kernel={args.median_kernel}) 边缘平滑度评估")
    print("="*60)
