...
核心思想：
1. 靠近左边界（前景-填补交界）：大权重平滑 → 消除垂直颜色跳变
2. 远离左边界（靠近真实背景）：小权重平滑 → 保持纹理几何结构
3. 右边缘羽化融合：消除填补-背景接缝
...
def parse_args():
    ...
    parser.add_argument("--smooth-kernel", type=int, default=7, help="垂直平滑核大小")
    parser.add_argument("--smooth-band-width", type=int, default=30, help="左边界平滑带宽度")
    parser.add_argument("--blend-width", type=int, default=4, help="右边缘羽化宽度")
...
# ========== v13: 渐变加权平滑 ==========
def inpaint_gradual_blend_smooth(image, hole_mask, target_near, bg_threshold=0.3,
                                  safety_margin=6, smooth_kernel=7, smooth_band_width=30,
                                  blend_width=4):
    """
    左边界附近的渐变加权平滑：
    1. 计算原采样坐标 sample_x_original
    2. 计算垂直平滑后的采样坐标 sample_x_smoothed
    3. 左边界附近（smooth_band_width）：alpha=1，完全使用平滑结果
       向右逐渐过渡：alpha -> 0，完全使用原结果
    4. 右边缘羽化融合
    """
    h, w = hole_mask.shape
    ...
    # ========== ✨ v13 核心改进：垂直平滑 + 渐变加权 ==========
    # 1. 先对整个 sample_x 场做垂直高斯平滑
    k = smooth_kernel
    pad = k // 2

    # 高斯核
    gauss_kernel = torch.exp(-(torch.arange(k, device=device, dtype=torch.float32) - pad) ** 2 / (2 * (k / 3) ** 2))
    gauss_kernel = gauss_kernel / gauss_kernel.sum()
    gauss_kernel = gauss_kernel.view(1, 1, k, 1)

    # 对 sample_x 做垂直高斯平滑
    sample_x_4d = sample_x_original.float().view(1, 1, h, w)
    sample_x_padded = F.pad(sample_x_4d, (0, 0, pad, pad), mode='replicate')
    sample_x_smoothed = F.conv2d(sample_x_padded, gauss_kernel)[0, 0].round().long().clamp(0, w - 1)
    ...
    # 3. 计算每个像素到左边界的距离，生成权重
    x_indices = torch.arange(w, device=device, dtype=torch.float32).view(1, w).expand(h, w)
    edge_x_2d = edge_x.view(h, 1).float().expand(h, w)
    dist_from_left_edge = x_indices - edge_x_2d

    # 权重 alpha：距离 = 0 → alpha = 1，距离 = smooth_band_width → alpha = 0
    alpha = (1.0 - dist_from_left_edge / smooth_band_width).clamp(0.0, 1.0)
    # 只在空洞区域内应用
    alpha = alpha * hole_mask.float()

    # 4. 加权混合原采样坐标和平滑后的采样坐标
    sample_x_blended = (alpha * sample_x_smoothed.float() + (1 - alpha) * sample_x_original.float()).round().long().clamp(0, w - 1)

    # 深度验证
    sample_right_known = torch.gather(known, 1, sample_x_blended)
    sample_right_near = torch.gather(target_near, 1, sample_x_blended)
    ...
    gather_x = sample_x_blended.unsqueeze(-1).expand(h, w, 3)
    ...
    if blend_width > 0:
        for y in range(h):
            ...
            alpha_blend = (dist_from_edge.float() / blend_width).clamp(0.0, 1.0).view(-1, 1)

            inpainted_colors = result[y, blend_x_range]
            bg_color = result[y, right_edge_x + 1]
            blended = alpha_blend * inpainted_colors + (1 - alpha_blend) * bg_color
            result[y, blend_x_range] = blended

    return result, sample_x_original, sample_x_smoothed, alpha
...
def process_frame(frame_bgr, model, device, args):
    ...
    final_right_v13, sample_x_orig, sample_x_smoothed, alpha = inpaint_gradual_blend_smooth(
        right_with_band, hole_with_band, target_near,
        bg_threshold=0.3, safety_margin=6,
        smooth_kernel=args.smooth_kernel,
        smooth_band_width=args.smooth_band_width,
    ...
    return {
        ...
        'final_v13': final_right_v13.cpu().numpy(),
...
def main():
    ...
    print(f"设备: {device}, 版本: v13 渐变加权平滑 (kernel={args.smooth_kernel}, band={args.smooth_band_width}, blend={args.blend_width}) ✨")
    ...
    for frame_idx in target_frames:
        ...
        cv2.imwrite(str(outdir / f'v13_frame_{frame_idx:03d}_b_original.png'),
                   cv2.cvtColor(final_b_u8, cv2.COLOR_RGB2BGR))

        final_v13_u8 = (result['final_v13'] * 255).astype(np.uint8)
        cv2.imwrite(str(outdir / f'v13_frame_{frame_idx:03d}_v13_improved.png'),
                   cv2.cvtColor(final_v13_u8, cv2.COLOR_RGB2BGR))
        ...
        for y in range(h):
            ...
            if len(cols) > 0:
                ...
                if right_edge_x + 1 < w:
                    ...
                    diff_v13 = np.abs(v13_result[y, right_edge_x] - v13_result[y, right_edge_x + 1]).mean()
                    boundary_diffs_b.append(diff_b)
                    boundary_diffs_v13.append(diff_v13)

        if len(boundary_diffs_b) > 0:
            print(f"\n填补-背景边界颜色差（右边缘）:")
            print(f"  B 原版 - 平均: {np.mean(boundary_diffs_b):.4f}, 最大: {np.max(boundary_diffs_b):.4f}")
            print(f"  v13 改进 - 平均: {np.mean(boundary_diffs_v13):.4f}, 最大: {np.max(boundary_diffs_v13):.4f}")
            impr = (1 - np.mean(boundary_diffs_v13) / np.mean(boundary_diffs_b)) * 100
        ...
        v13_crop = v13_result[y1:y2, x1:x2]

        b_grad_v = np.abs(b_crop[1:, :] - b_crop[:-1, :]).mean(axis=2)
        v13_grad_v = np.abs(v13_crop[1:, :] - v13_crop[:-1, :]).mean(axis=2)

        print(f"\n填补区域垂直颜色平滑度:")
        print(f"  B 原版 - 平均颜色差: {b_grad_v.mean():.4f}")
        print(f"  v13 改进 - 平均颜色差: {v13_grad_v.mean():.4f}")
        impr_color = (1 - v13_grad_v.mean() / max(b_grad_v.mean(), 1e-6)) * 100
        print(f"  颜色平滑改进: {impr_color:.1f}%")

        # 只看左边界附近的垂直平滑（前 30 像素）
        hole_left_edge_diffs_b = []
        hole_left_edge_diffs_v13 = []
        for y in range(h - 1):
            ...
            if len(cols_y) > 0 and len(cols_y1) > 0:
                ...
                for dx in range(0, min(30, len(cols_y), len(cols_y1))):
                    if left_x + dx < w - 1:
                        diff_b = np.abs(b_result[y, left_x + dx] - b_result[y + 1, left_x + dx]).mean()
                        diff_v13 = np.abs(v13_result[y, left_x + dx] - v13_result[y + 1, left_x + dx]).mean()
                        hole_left_edge_diffs_b.append(diff_b)
                        hole_left_edge_diffs_v13.append(diff_v13)

        if len(hole_left_edge_diffs_b) > 0:
            print(f"\n左边界附近 30 像素垂直平滑度:")
            print(f"  B 原版 - 平均: {np.mean(hole_left_edge_diffs_b):.4f}")
            print(f"  v13 改进 - 平均: {np.mean(hole_left_edge_diffs_v13):.4f}")
            impr_left = (1 - np.mean(hole_left_edge_diffs_v13) / np.mean(hole_left_edge_diffs_b)) * 100
            print(f"  左边界改进: {impr_left:.1f}%")

    print(f"\n{'='*60}")
    print(f"✅ v13 渐变加权平滑完成！结果保存在: {outdir}")
    print(f"   对比: *_b_original.png vs *_v13_improved.png")
