...
结合所有最佳方案：
1. 保守反遮挡带 - 减少过度扩散，保护手部等前景细节
2. 填补采样坐标垂直中值滤波 - 消除垂直方向颜色跳变（v7 核心思想）
3. 填补-背景边界羽化融合 - 消除接缝（v10 核心思想）
...
def parse_args():
    ...
    parser.add_argument("--smooth-kernel", type=int, default=7, help="采样坐标垂直平滑核大小")
    parser.add_argument("--blend-width", type=int, default=6, help="边界羽化宽度")
...
# ========== 2. 改进填补：采样坐标平滑 + 边界羽化 ==========
def inpaint_improved(image, hole_mask, target_near, bg_threshold=0.3,
                     safety_margin=6, smooth_kernel=7, blend_width=6):
    """
    综合优化的填补算法：
    1. 正常 B 版本填补流程
    2. 对填补采样坐标做垂直中值滤波（改进垂直连续性）
    3. 填补后对右边缘做羽化融合（消除接缝）
    """
    h, w = hole_mask.shape
    ...
    unfolded = unfolded.view(k, h, w)
    sample_x_smoothed = torch.median(unfolded, dim=0)[0].round().long().clamp(0, w - 1)
    ...
    if blend_width > 0:
        for y in range(h):
            ...
            alpha = (dist_from_edge.float() / blend_width).clamp(0.0, 1.0)
    ...
    return result, sample_x_original, sample_x_smoothed
...
def process_frame(frame_bgr, model, device, args):
    ...
    final_right_v11, sample_x_orig, sample_x_smoothed = inpaint_improved(
    ...
    return {
        ...
        'final_v11': final_right_v11.cpu().numpy(),
...
def main():
    ...
    print(f"设备: {device}, 版本: v11 最终综合优化 (smooth={args.smooth_kernel}, blend={args.blend_width}, cleanup={args.band_cleanup}) ✨")
    ...
    for frame_idx in target_frames:
        ...
        cv2.imwrite(str(outdir / f'v11_frame_{frame_idx:03d}_b_original.png'),
                   cv2.cvtColor(final_b_u8, cv2.COLOR_RGB2BGR))

        final_v11_u8 = (result['final_v11'] * 255).astype(np.uint8)
        cv2.imwrite(str(outdir / f'v11_frame_{frame_idx:03d}_v11_improved.png'),
                   cv2.cvtColor(final_v11_u8, cv2.COLOR_RGB2BGR))

        # 反遮挡带可视化
        band_viz = (result['warped'] * 255).astype(np.uint8)
        band_viz[result['band']] = [0, 255, 0]
        cv2.imwrite(str(outdir / f'v11_frame_{frame_idx:03d}_band.png'),
        ...
        for y in range(h):
            ...
            if len(cols) > 0:
                ...
                if right_edge_x + 1 < w:
                    ...
                    diff_v11 = np.abs(v11_result[y, right_edge_x] - v11_result[y, right_edge_x + 1]).mean()
                    boundary_diffs_b.append(diff_b)
                    boundary_diffs_v11.append(diff_v11)

        if len(boundary_diffs_b) > 0:
            print(f"\n填补-背景边界颜色差（右边缘）:")
            print(f"  B 原版 - 平均: {np.mean(boundary_diffs_b):.4f}, 最大: {np.max(boundary_diffs_b):.4f}")
            print(f"  v11 改进 - 平均: {np.mean(boundary_diffs_v11):.4f}, 最大: {np.max(boundary_diffs_v11):.4f}")
            impr = (1 - np.mean(boundary_diffs_v11) / np.mean(boundary_diffs_b)) * 100
        ...
        v11_crop = v11_result[y1:y2, x1:x2]

        b_grad_v = np.abs(b_crop[1:, :] - b_crop[:-1, :]).mean(axis=2)
        v11_grad_v = np.abs(v11_crop[1:, :] - v11_crop[:-1, :]).mean(axis=2)

        print(f"\n填补区域垂直颜色平滑度:")
        print(f"  B 原版 - 平均颜色差: {b_grad_v.mean():.4f}")
        print(f"  v11 改进 - 平均颜色差: {v11_grad_v.mean():.4f}")
        impr_color = (1 - v11_grad_v.mean() / max(b_grad_v.mean(), 1e-6)) * 100
        print(f"  颜色平滑改进: {impr_color:.1f}%")

    print(f"\n{'='*60}")
    print(f"✅ v11 最终综合优化完成！结果保存在: {outdir}")
    print(f"   对比: *_b_original.png vs *_v11_improved.png")
