"""
迭代 v40: 空洞向右膨胀 + 严格右侧填充

核心修复:
  问题：空洞右侧残留前景像素 → 从右向左填充时取到前景颜色
  方案：
    1. 对空洞进行向右定向膨胀 N 像素（覆盖残留的前景）
    2. 然后严格右侧填充

版本目录: v40_dilate{N}_kernel{K}
"""
import sys
import cv2
import numpy as np
...
def dilate_hole_right(hole, dilate_pixels, device):
    ...
    dilate_pixels: 向右膨胀的像素数
    """
    h, w = hole.shape
    hole_dilated = hole.clone()

    # 从右向左扫描，遇到空洞就把左边N个也设为空洞
    # 不对，应该是从左向右，遇到空洞就把右边N个设为空洞
    for i in range(1, dilate_pixels + 1):
        shifted = torch.roll(hole, shifts=i, dims=1)
        shifted[:, :i] = False  # 左边缘不膨胀
...
def main_debug_frame():
    ...
    print(f"[v40] 处理第 70 秒 → 第 {target_frame} 帧")
    ...
    print(f"[v40] 原始空洞: {hole.sum().item()} 像素")

    # 测试不同膨胀量 + 不同核尺寸
    for dilate_pixels in [2, 4, 8, 12, 16]:
        # 向右膨胀空洞
        hole_dilated = dilate_hole_right(hole, dilate_pixels, device)
        dilated_count = hole_dilated.sum().item()
        print(f"\n[v40] 向右膨胀 {dilate_pixels} 像素 → {dilated_count:,} 像素 "
        ...
        for kernel_size in [11, 15]:
            subdir = Path(f"/mnt/A/jiangxg/work/2Dto3D/edge_smooth_iterations/frames/"
                         f"v40_dilate{dilate_pixels}_kernel{kernel_size}")
            ...
            result, steps = inpaint_strict_right(
                ...
                kernel_size=kernel_size, max_iter=30
            )
            t_v40 = time.time() - t0
            print(f"         耗时: {t_v40*1000:.1f} ms, {len(steps)-1} 次迭代")
