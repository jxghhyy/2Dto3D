"""
迭代 v43: 正确的向右膨胀 + 严格右侧填充

核心修复:
  1. 正确的逐行向右膨胀逻辑
  2. 统一输出文件夹结构：v43/dilate{N}_kernel{K}/

版本目录: v43/dilate_right{N}_kernel{K}/
"""
import sys
import cv2
import numpy as np
...
BASE_OUT_DIR = Path("/mnt/A/jiangxg/work/2Dto3D/edge_smooth_iterations/frames/v43")
...
def main_debug_frame():
    ...
    print(f"[v43] 使用设备: {device}")
    print(f"[v43] 输出根目录: {BASE_OUT_DIR}")
    ...
    print(f"[v43] 处理第 70 秒 → 第 {target_frame} 帧")
    ...
    print(f"[v43] 原始空洞: {hole.sum().item()} 像素")

    # 测试不同向右膨胀量
    for dilate_pixels in [0, 2, 4, 6, 8, 11]:
        # 向右膨胀空洞（可控参数: dilate_pixels）
        if dilate_pixels == 0:
            hole_dilated = hole
        else:
            hole_dilated = dilate_hole_right(hole, dilate_pixels, device)
        dilated_count = hole_dilated.sum().item()
        print(f"\n[v43] 向右膨胀 {dilate_pixels} 像素 → {dilated_count:,} 像素 "
        ...
        for kernel_size in [11, 15]:
            ...
            t_v43 = time.time() - t0
            print(f"         耗时: {t_v43*1000:.1f} ms, {len(steps)-1} 次迭代")
