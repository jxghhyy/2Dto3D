...
核心洞察：
B 版本的镜像机制是完美的 - 它保留了背景几何结构（天花板灯不会扭曲）。
但问题是：每行的空洞左边界 edge_x 不同，导致镜像的起点不同。

v9 解决方案（更优雅）：
1. 找到每行的空洞左边界 edge_x[y]
2. 对 edge_x[y] 做垂直平滑，得到 smoothed_edge_x[y]
3. 计算偏移量：delta[y] = smoothed_edge_x[y] - edge_x[y]
4. 对这一行空洞内**所有像素**的取色位置，整体偏移 delta[y]

结果：
- 空洞内像素之间的**相对位置保持不变**（几何结构保留）
- 相邻行的取色起点是连续的（颜色平滑）
- 完美！
"""
import sys
...
def parse_args():
    ...
    parser.add_argument("--smooth-kernel", type=int, default=7, help="边界垂直平滑核大小")
    parser.add_argument("--band-cleanup", type=int, default=10, help="反遮挡带清理宽度")
    parser.add_argument("--min-drop", type=float, default=3.5, help="视差下降阈值")
    return parser.parse_args()


# ========== 保守的反遮挡带（和 v8 一样） ==========
def project_disocclusion_bands_conservative(disparity, min_drop=3.5, right_cleanup=10):
    h, w = disparity.shape
    if w < 2:
    ...
    is_significant_edge = disp_grad >= (grad_mean + 0.3 * grad_std)
...
# ========== v9 核心：边界偏移平滑法 ==========
def inpaint_with_edge_offset_smoothing(image, hole_mask, target_near, bg_threshold=0.3,
                                       safety_margin=6, smooth_kernel=7):
    """
    ✨ 核心创新：边界偏移平滑法

    步骤：
    1. 和 B 版本完全一样计算每个像素的取色位置 sample_x
    2. 找到每行的空洞左边界 edge_x[y]
    3. 对 edge_x[y] 做垂直中值滤波，得到 smoothed_edge_x[y]
    4. 计算偏移量：delta[y] = smoothed_edge_x[y] - edge_x[y]
    5. 对每行所有空洞内的 sample_x，整体加上 delta[y]

    关键性质：
    - delta[y] 对整行是同一个常数
    - 所以空洞内像素之间的相对取色位置完全不变！
    - 几何结构 100% 保留，同时相邻行的颜色是连续的！
    """
    h, w = hole_mask.shape
    ...
    for y in range(h):
        ...
        if len(cols) > 0:
            edge_x[y] = cols.min()  # 这一行最左边的空洞像素 = 边界位置
    ...
    if valid_rows.sum() > smooth_kernel:
        ...
        k = smooth_kernel
        pad = k // 2
        edge_x_float = edge_x.float().view(1, 1, h, 1)
        edge_x_padded = F.pad(edge_x_float, (0, 0, pad, pad), mode='replicate')

        unfolded = F.unfold(edge_x_padded, kernel_size=(k, 1), padding=0, stride=1)
        unfolded = unfolded.squeeze(0)
        edge_x_smoothed = torch.median(unfolded, dim=0)[0].round().long()

        # 计算偏移量 delta
        delta = edge_x_smoothed - edge_x

        # 限制偏移量，避免引入错误
        max_delta = 5  # 最多偏移 5 个像素
        delta = delta.clamp(-max_delta, max_delta)

        # 对每行的所有 sample_x 应用相同的偏移
        # 关键：整行的相对取色位置完全不变！
        sample_x_smoothed = sample_x_original + delta.view(h, 1)
    ...
    right_ok = right_ok_geometry & right_depth_ok & sample_right_known
    ...
    return result, sample_x_original, sample_x_smoothed, edge_x, delta
...
def process_frame(frame_bgr, model, device, args):
    ...
    final_right_v9, sample_x_orig, sample_x_smoothed, edge_x, delta = inpaint_with_edge_offset_smoothing(
...
def main():
    ...
    print(f"设备: {device}, 版本: v9 边界偏移平滑法 (kernel={args.smooth_kernel}, cleanup={args.band_cleanup}) ✨")
    ...
    for frame_idx in target_frames:
        ...
        cv2.imwrite(str(outdir / f'v9_frame_{frame_idx:03d}_b_original.png'),
                   cv2.cvtColor(final_b_u8, cv2.COLOR_RGB2BGR))

        # 2. v9 改进
        final_v9_u8 = (result['final_v9'] * 255).astype(np.uint8)
        cv2.imwrite(str(outdir / f'v9_frame_{frame_idx:03d}_v9_improved.png'),
                   cv2.cvtColor(final_v9_u8, cv2.COLOR_RGB2BGR))

        # 3. 反遮挡带可视化
        band_viz = (result['warped'] * 255).astype(np.uint8)
        band_viz[result['band']] = [0, 255, 0]
        cv2.imwrite(str(outdir / f'v9_frame_{frame_idx:03d}_band.png'),
                   cv2.cvtColor(band_viz, cv2.COLOR_RGB2BGR))

        # 4. 空洞可视化
        hole_viz = (result['warped'] * 255).astype(np.uint8)
        hole_viz[result['hole_with_band']] = [0, 255, 0]
        cv2.imwrite(str(outdir / f'v9_frame_{frame_idx:03d}_hole.png'),
        ...
        if valid.sum() > 0:
            ...
            print(f"\n边界偏移量 delta:")
            print(f"  平均: {np.abs(delta_valid).mean():.2f}px")
            print(f"  最大: {np.abs(delta_valid).max():.0f}px")
            print(f"  标准差: {delta_valid.std():.2f}")

        # 取色位置平滑度
        hole_mask = result['hole_with_band']
        if hole_mask.sum() > 0:
            sample_x_orig = result['sample_x_orig']
            sample_x_smoothed = result['sample_x_smoothed']

            grad_orig = np.abs(sample_x_orig[1:, :] - sample_x_orig[:-1, :])
            ...
            if hole_grad_mask.sum() > 0:
                print(f"\n取色位置垂直平滑度（空洞区域）:")
                print(f"  原版 - 平均: {grad_orig[hole_grad_mask].mean():.2f}px, 最大: {grad_orig[hole_grad_mask].max():.0f}px")
                print(f"  平滑 - 平均: {grad_smoothed[hole_grad_mask].mean():.2f}px, 最大: {grad_smoothed[hole_grad_mask].max():.0f}px")
                impr = (1 - grad_smoothed[hole_grad_mask].mean() / max(grad_orig[hole_grad_mask].mean(), 1e-6)) * 100
                print(f"  改进: {impr:.1f}%")
        ...
        b_crop = result['final_b'][y1:y2, x1:x2]
        v9_crop = result['final_v9'][y1:y2, x1:x2]

        b_grad_v = np.abs(b_crop[1:, :] - b_crop[:-1, :]).mean(axis=2)
        v9_grad_v = np.abs(v9_crop[1:, :] - v9_crop[:-1, :]).mean(axis=2)

        print(f"\n填补区域垂直颜色平滑度:")
        print(f"  B 原版 - 平均颜色差: {b_grad_v.mean():.4f}")
        print(f"  v9 改进 - 平均颜色差: {v9_grad_v.mean():.4f}")
        impr_color = (1 - v9_grad_v.mean() / max(b_grad_v.mean(), 1e-6)) * 100
        print(f"  颜色平滑改进: {impr_color:.1f}%")

    print(f"\n{'='*60}")
    print(f"✅ v9 迭代完成！结果保存在: {outdir}")
    print(f"   对比: *_b_original.png vs *_v9_improved.png")
