...
策略：
1. 先用正常参数生成反遮挡带（不那么严格的约束，确保有带生成）
2. 然后找到前景轮廓
3. 只保留轮廓线右侧的反遮挡带（防止渗透到前景内部）

这样既保证有反遮挡带生成，又不会渗透到前景内部
"""
import sys
import argparse
import cv2
...
# ========== 1. 正常生成反遮挡带，然后用前景边界后过滤 ==========
def project_disocclusion_bands_post_filter(
    disparity, near_score, warp_valid_mask,
    min_drop=3.0, right_cleanup=10
):
    """
    简单策略：
    1. 正常生成反遮挡带（和 v15 一样的参数）
    2. 找到前景轮廓线（warp 后有效像素的最右边缘）
    3. 只保留轮廓线右侧 ± small_offset 的带
    """
    h, w = disparity.shape
    if w < 2:
    ...
    is_significant_edge = disp_grad >= (grad_mean * 0.3)  # 0.3 -> 更宽松
    ...
    # ========== 步骤 C：✨ 后过滤 - 只保留前景边界附近的带 ✨ ==========
    x_full = torch.arange(w, device=device).view(1, w).expand(h, w)
    foreground_edge_full = foreground_right_edge.view(h, 1).expand(h, w)

    # 只保留：前景边界左侧 30 像素 到 右侧 整个区域的带
    # 这样带不会渗透到前景内部太远的地方
    allowed_left_of_edge = 30  # 允许在边界左侧 30 像素内有带
    keep_band = x_full >= (foreground_edge_full - allowed_left_of_edge)
...
# ========== 3. 优化的填补算法 ==========
def inpaint_v19_optimized(image, hole_mask, target_near, near_score,
...
def process_frame(frame_bgr, model, device, args):
    ...
    # ========== v19：后过滤反遮挡带 ==========
    disocclusion_band, foreground_right_edge = project_disocclusion_bands_post_filter(
    ...
    final_right_optimized = inpaint_v19_optimized(
...
def main():
    ...
    print(f"设备: {device}, 版本: v19 反遮挡带后过滤 ✨")
    ...
    for frame_idx in test_frames:
        ...
        cv2.imwrite(str(outdir / f'v19_frame_{frame_idx:03d}_optimized.png'),
                   cv2.cvtColor(opt_u8, cv2.COLOR_RGB2BGR))

        # 反遮挡带可视化
        band_viz = (result['warped'] * 255).astype(np.uint8)
        band_viz[result['band']] = [0, 255, 0]
        cv2.imwrite(str(outdir / f'v19_frame_{frame_idx:03d}_band_green.png'),
        ...
        cv2.imwrite(str(outdir / f'v19_frame_{frame_idx:03d}_foreground_edge.png'),
        ...
        cv2.imwrite(str(outdir / f'v19_frame_{frame_idx:03d}_hand_crop.png'), hand_crop)
