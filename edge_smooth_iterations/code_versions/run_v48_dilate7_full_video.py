...
KERNEL_SIZE = 15
LEFT_WIDTH = 3
MAX_DILATE = 7  # v48: 从 5 改成 7
MAX_ITER = 30
VIDEO_PATH = "/mnt/A/jiangxg/dataset/shuai/Huawei/badminton_2min.mp4"
OUT_DIR = Path("/mnt/A/jiangxg/work/2Dto3D/edge_smooth_iterations/frames/v48_dilate7_asym_full")
...
def main():
    ...
    print(f"[v48 完整视频] 使用设备: {device}")
    print(f"[v48 完整视频] 参数: 不对称核 size={KERNEL_SIZE}, left_width={LEFT_WIDTH}")
    print(f"[v48 完整视频] GPU 自适应膨胀 max_dilate={MAX_DILATE}")
    print(f"[v48 完整视频] 输出目录: {OUT_DIR}")
    ...
    print(f"[v48 完整视频] 视频: {fps:.1f} fps, {total_frames} 帧, {w_orig}×{h_orig}")

    out_path = OUT_DIR / f"v48_dilate{MAX_DILATE}_asym_left{LEFT_WIDTH}_kernel{KERNEL_SIZE}.mp4"
    sbs_out_path = OUT_DIR / f"v48_dilate{MAX_DILATE}_asym_left{LEFT_WIDTH}_kernel{KERNEL_SIZE}_sbs.mp4"
    ...
    for frame_idx in pbar:
        ...
        hole_dilated = dilate_hole_right_adaptive_gpu(hole, MAX_DILATE, max_disparity, device)
