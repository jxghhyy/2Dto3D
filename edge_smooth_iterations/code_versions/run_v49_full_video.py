"""
v49 完整视频：基于颜色相似度的空洞膨胀 + 不对称核填充

✨ 核心改进：
1. 膨胀：基于颜色相似度检测前景残留，不是盲目膨胀
2. 填充：left_width=5 + max_iter=8，比 v47 快 ~50ms
"""
import sys
import cv2
import numpy as np
...
# ========== 参数 ==========
KERNEL_SIZE = 15
LEFT_WIDTH = 5        # 从 3 增加到 5，填充更快
MAX_ITER = 8          # 实际只需要 4 次，留余量
COLOR_THRESHOLD = 0.08  # 颜色相似度阈值
MAX_DILATE = 8        # 最多膨胀 8 像素
VIDEO_PATH = "/mnt/A/jiangxg/dataset/shuai/Huawei/badminton_2min.mp4"
OUT_DIR = Path("/mnt/A/jiangxg/work/2Dto3D/edge_smooth_iterations/frames/v49_color_dilate_full")
...
@torch.no_grad()
def dilate_by_color_similarity(right_warped, hole, max_dilate=8, color_threshold=0.08, device='cuda'):
    """
    基于颜色相似度的空洞膨胀算法

    原理：
    1. 对每行找到所有连续空洞区域 [start_x, end_x]
    2. 参考颜色 = 空洞左边第一个非空洞像素
...
def main():
    ...
    print(f"[v49 完整视频] 设备: {device}")
    print(f"[v49 完整视频] 参数: 不对称核 size={KERNEL_SIZE}, left_width={LEFT_WIDTH}")
    print(f"[v49 完整视频] 颜色膨胀: threshold={COLOR_THRESHOLD}, max_dilate={MAX_DILATE}")
    print(f"[v49 完整视频] 最大迭代: {MAX_ITER}")
    print(f"[v49 完整视频] 输出目录: {OUT_DIR}")
    ...
    print(f"[v49 完整视频] 视频: {fps:.1f} fps, {total_frames} 帧, {w_orig}×{h_orig}")

    out_path = OUT_DIR / f"v49_color_th{int(COLOR_THRESHOLD*100)}_left{LEFT_WIDTH}.mp4"
    sbs_out_path = OUT_DIR / f"v49_color_th{int(COLOR_THRESHOLD*100)}_left{LEFT_WIDTH}_sbs.mp4"
    ...
    for frame_idx in pbar:
        ...
        # v49: 颜色相似度膨胀
        t_dilate = time.time()
        hole_dilated = dilate_by_color_similarity(
