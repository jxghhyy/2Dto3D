"""
迭代 v42: 修复向右膨胀逻辑 + 严格右侧填充

核心修复:
  问题：空洞右侧残留前景像素条带 → 严格右侧填充时取到前景颜色
  方案：
    1. 正确向右膨胀空洞（覆盖右侧前景残留）
    2. 严格右侧填充

版本目录: v42_dilate_right{N}_kernel{K}
"""
import sys
import cv2
import numpy as np
...
@torch.no_grad()
def dilate_hole_right_correct(hole, dilate_pixels, device):
    """
    正确的向右膨胀：从左向右扫描，遇到空洞就把右边N个也设为空洞
    这样空洞边界会向右移动 N 像素，覆盖右侧的前景残留
    """
    h, w = hole.shape
...
@torch.no_grad()
def inpaint_strict_right(img, hole, kernel_size=11, max_iter=30):
    """
    严格右侧填充
    """
    result = img.clone()
...
def main_debug_frame():
    ...
    print(f"[v42] 处理第 70 秒 → 第 {target_frame} 帧")
    ...
    print(f"[v42] 原始空洞: {hole.sum().item()} 像素")

    # 测试不同向右膨胀量
    for dilate_pixels in [3, 5, 8, 11, 15]:
        # 向右膨胀空洞（正确版本）
        hole_dilated = dilate_hole_right_correct(hole, dilate_pixels, device)
        dilated_count = hole_dilated.sum().item()
        print(f"\n[v42] 向右膨胀 {dilate_pixels} 像素 → {dilated_count:,} 像素 "
        ...
        for kernel_size in [11, 15]:
            subdir = Path(f"/mnt/A/jiangxg/work/2Dto3D/edge_smooth_iterations/frames/"
                         f"v42_dilate_right{dilate_pixels}_kernel{kernel_size}")
            ...
            t_v42 = time.time() - t0
            print(f"         耗时: {t_v42*1000:.1f} ms, {len(steps)-1} 次迭代")
