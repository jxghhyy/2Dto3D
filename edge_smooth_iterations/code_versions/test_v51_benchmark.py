...
def dilate_v47(right_warped, hole, max_dilate=5, color_threshold=0.08):
    """v47: CPU 只向右"""
    h, w = hole.shape
...
def dilate_v51(right_warped, hole, max_dilate=8, color_threshold=0.15):
    """v51: CPU 只向右，阈值放宽"""
    h, w = hole.shape
    hole_np = hole.cpu().numpy()
    hole_dilated = hole_np.copy()
...
def main():
    ...
    for _ in range(20):
        t0 = time.time()
        h_v47 = dilate_v47(right_warped, hole, 5, 0.08)
        times.append((time.time() - t0) * 1000)
    count_v47 = h_v47.sum() - original_count
    print(f"  v47 (只向右, 0.08, 5px): {np.mean(times):.1f} ms, 新增 {count_v47:,} 像素")

    # v51
    times = []
    for _ in range(20):
        t0 = time.time()
        h_v51 = dilate_v51(right_warped, hole, 8, 0.15)
        times.append((time.time() - t0) * 1000)
    count_v51 = h_v51.sum() - original_count
    print(f"  v51 (只向右, 0.15, 8px): {np.mean(times):.1f} ms, 新增 {count_v51:,} 像素")

    print(f"\n✅ 性能对比完成！")
    print(f"  耗时增加: {np.mean(times):.1f}ms (v51) vs 之前 v50 的 119ms")
    print(f"  原因: v50 做了三个方向，v51 只做一个方向，但是还有优化空间")
