...
策略（简化版）：
1. 全局从右向左填充，核从 11 开始，每轮减 2，直到 3（共 5 轮）
   - 从右向左用背景填充，防止前景颜色泄漏
   - 核逐渐减小，越往里面填充越精细
2. 填充完成后，检测剩余空洞（应该只有最右侧的区域）
   - 对剩余空洞，从左向右填充（核=3，不设上限）
...
@torch.no_grad()
def fill_hole_simple(img, hole, device):
    """
    简化版两阶段填充：
    1. 从右向左，核从11递减到3
    2. 剩余空洞从左向右，不设上限
    ...
    # ========== 阶段1: 从右向左填充，核逐渐减小 ==========
    print(f"\n  阶段1: 从右向左填充（核 {INITIAL_KERNEL} → {MIN_KERNEL}）")
    ...
    while kernel_size >= MIN_KERNEL:
        ...
        print(f"    核={kernel_size}, 填充={filled}像素, 剩余={hole_cur.sum().item()}")

        # 核减小
        kernel_size -= 2

    print(f"  阶段1完成: 共{stage1_iter}轮迭代")

    # ========== 阶段2: 剩余空洞从左向右填充 ==========
    remaining = hole_cur.sum().item()
    print(f"\n  阶段2: 剩余 {remaining} 像素从左向右填充（核=3，无上限）")
    ...
    while True:
        ...
        if stage2_iter % 5 == 0:
            print(f"    已迭代{stage2_iter}次, 剩余={hole_cur.sum().item()}")

    print(f"  阶段2完成: 共{stage2_iter}轮迭代, 最终剩余={hole_cur.sum().item()}")
...
def process_frame(frame_idx, frame_bgr, model, device, frame_name):
    ...
    # v56 简化版填充
    result, hole_final = fill_hole_simple(right_with_hole, hole_dilated, device)
    ...
    cv2.imwrite(str(OUT_DIR / f"{frame_name}_simple_result.png"), result_bgr)
    cv2.imwrite(str(OUT_DIR / f"{frame_name}_simple_combined.png"), combined)

    print(f"\n结果已保存: {frame_name}_simple_result.png / {frame_name}_simple_combined.png")
    return result_bgr
...
def main():
    ...
    print(f"[v56简化版] 设备: {device}")
    print(f"[v56简化版] 初始核={INITIAL_KERNEL}, 最小核={MIN_KERNEL}")
    print(f"[v56简化版] 输出目录: {OUT_DIR}")
