...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--inpaint-mode", choices=["fast", "partialconv", "hybrid"], default="fast",
                        help="补洞模式：fast=原边缘敏感补洞；partialconv=PartialConv风格GPU补洞；hybrid=PartialConv后FAST兜底")
    parser.add_argument("--pconv-kernel-size", type=int, default=7, help="PartialConv补洞核大小(奇数)")
    parser.add_argument("--pconv-max-iter", type=int, default=32, help="PartialConv补洞最大迭代次数")
    parser.add_argument("--pconv-depth-weight", type=float, default=1.0, help="PartialConv背景深度权重强度，0=不使用深度偏置")
    parser.add_argument("--pconv-fallback", choices=["fast", "mean", "none"], default="fast",
                        help="PartialConv剩余空洞兜底：fast=调用FAST；mean=均值填充；none=不兜底")
...
def partialconv_inpaint_gpu(
    ...
    if kernel_size % 2 == 0:
        raise ValueError("pconv-kernel-size必须是奇数")
    ...
    device = img.device
    if not torch.any(hole):
        _stage_add(stage_times, "pconv_skip_no_holes", time.perf_counter() - t0)
        return img, hole
    ...
    _maybe_sync(device, profile_sync)
    _stage_add(stage_times, "pconv_prepare_masks", time.perf_counter() - t0)
    ...
    for _ in range(max_iter):
        ...
        valid = (~hole).to(img.dtype)
        if near is not None and depth_weight > 0.0:
            bg = (near < bg_threshold).to(img.dtype)
            source_w = valid * ((1.0 - depth_weight) + depth_weight * bg)
            if not torch.any(source_w > 0):
                source_w = valid
        else:
            source_w = valid

        source_w_nchw = source_w.unsqueeze(0).unsqueeze(0)
        weight_sum = F.conv2d(source_w_nchw, kernel1, padding=pad)
        fillable = hole & (weight_sum[0, 0] > 0.01)
        if not torch.any(fillable):
            break

        img_nchw = img.permute(2, 0, 1).unsqueeze(0)
        rgb_sum = F.conv2d(img_nchw * source_w_nchw, kernel3, padding=pad, groups=3)
        avg = rgb_sum / weight_sum.clamp_min(1e-6)
        avg_hwc = avg[0].permute(1, 2, 0)
    ...
    _maybe_sync(device, profile_sync)
    _stage_add(stage_times, f"pconv_iter_{iter_count}iter", time.perf_counter() - t0)
    return img, hole


def fill_remaining_with_mean(image: torch.Tensor, hole_mask: torch.Tensor, original_hole: torch.Tensor) -> torch.Tensor:
    if not torch.any(hole_mask):
        return image
    img = image.clone()
    known_pixels = img[~original_hole]
    fallback = known_pixels.mean(dim=0) if known_pixels.numel() > 0 else torch.zeros(3, device=img.device, dtype=img.dtype)
    img[hole_mask] = fallback
    return img
...
def main() -> None:
    ...
    print(f"[mono2stereo] 🎨 补洞模式: {args.inpaint_mode}")
    print(f"[mono2stereo] 🎨 FAST边缘敏感补洞: {mode_names[args.edge_fill_mode]} (edge_kernel={args.edge_kernel_size}, non_edge_kernel={args.non_edge_kernel_size}, bg_threshold={args.bg_threshold})")
    if args.inpaint_mode in ("partialconv", "hybrid"):
        print(f"[mono2stereo] 🎨 PartialConv-style: kernel={args.pconv_kernel_size}, max_iter={args.pconv_max_iter}, depth_weight={args.pconv_depth_weight}, fallback={args.pconv_fallback}")
    ...
    for idx, filename in enumerate(files):
        ...
        try:
            while True:
                ...
                if args.inpaint_mode == "fast":
                    ...
                    t0 = time.perf_counter()
                    right_inpainted_dibr, remaining_hole = partialconv_inpaint_gpu(
                        right_rgb_dibr, hole_mask=hole_dilated,
                        kernel_size=args.pconv_kernel_size, max_iter=args.pconv_max_iter,
                        stage_times=stage_times, profile_sync=args.profile_time,
                        near=near_for_inpaint,
                        bg_threshold=args.bg_threshold,
                        depth_weight=args.pconv_depth_weight,
                    )
                    if args.profile_time:
                        torch.cuda.synchronize()
                    _stage_add(stage_times, "partialconv_inpaint_total", time.perf_counter() - t0)

                    if torch.any(remaining_hole) and (args.inpaint_mode == "hybrid" or args.pconv_fallback == "fast"):
                        t0 = time.perf_counter()
                        right_inpainted_dibr = fast_inpaint_gpu(
                            right_inpainted_dibr, hole_mask=remaining_hole,
                            kernel_size=args.fast_kernel, max_iter=args.fast_max_iter,
                            stage_times=stage_times, profile_sync=args.profile_time,
                            near=near_for_inpaint,
                            bg_threshold=args.bg_threshold,
                        ...
                        if args.profile_time:
                            torch.cuda.synchronize()
                        _stage_add(stage_times, "partialconv_fallback_total", time.perf_counter() - t0)
                    elif torch.any(remaining_hole) and args.pconv_fallback == "mean":
                        t0 = time.perf_counter()
                        right_inpainted_dibr = fill_remaining_with_mean(right_inpainted_dibr, remaining_hole, hole_dilated)
                        if args.profile_time:
                            torch.cuda.synchronize()
                        _stage_add(stage_times, "partialconv_fallback_total", time.perf_counter() - t0)
        ...
        if args.profile_time and processed_frames > 0:
            ...
            top_level = [
                ...
                ("fast_inpaint_total", "⚡ FAST补洞总耗时 (深度感知版)"),
                ("partialconv_inpaint_total", "⚡ PartialConv-style补洞总耗时"),
                ("partialconv_fallback_total", "PartialConv剩余空洞兜底"),
            ...
            if "partialconv_inpaint_total" in stage_times:
                parent_time = stage_times["partialconv_inpaint_total"]
                parent_avg = parent_time / processed_frames * 1000
                profile_lines.append(f"\n[partialconv_inpaint_total] 内部拆解 (总: {parent_time:.3f}s, 平均: {parent_avg:.1f}ms/帧):")
                pconv_keys = [k for k in stage_times if k.startswith("pconv_")]
                for key in sorted(pconv_keys):
                    t = stage_times[key]
                    t_avg = t / processed_frames * 1000
                    ratio = (t / parent_time * 100.0) if parent_time > 0 else 0.0
                    if key == "pconv_skip_no_holes":
                        cn_name = "跳过 (无空洞)"
                    elif key == "pconv_prepare_masks":
                        cn_name = "准备mask和卷积核"
                    elif key.startswith("pconv_iter_"):
                        iters = key.replace("pconv_iter_", "").replace("iter", "")
                        cn_name = f"PartialConv迭代 ({iters}次)"
                    else:
                        cn_name = ""
                    profile_lines.append(f"  ├─ {key:25s} {t:8.3f}s ({ratio:5.1f}%) | {t_avg:6.1f}ms/帧 | {cn_name}")
            ...
            try:
                ...
                with open(profile_file, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"inpaint_mode: {args.inpaint_mode}\n")
                    ...
                    f.write(f"pconv_kernel_size: {args.pconv_kernel_size}\n")
                    f.write(f"pconv_max_iter: {args.pconv_max_iter}\n")
                    f.write(f"pconv_depth_weight: {args.pconv_depth_weight}\n")
                    f.write(f"pconv_fallback: {args.pconv_fallback}\n")
